// Alma Browser Relay - Background Service Worker
// Connects Chrome to Alma via WebSocket for AI-powered browser control

let ws = null;
let reconnectDelay = 1000;
const MAX_RECONNECT_DELAY = 30000;
const KEEPALIVE_INTERVAL_NAME = 'alma-keepalive';
...
async function loadConfig() {
  ...
  try {
    ...
    console.warn('[Alma Relay] Failed to load config from storage:', e);
  }

  // Also try auto-config from Alma server
  try {
...
function connect(silent = false) {
  ...
  try {
    ...
    console.error('[Alma Relay] Failed to create WebSocket:', e);
  ...
  ws.onopen = () => {
    console.log('[Alma Relay] Connected to Alma server');
  ...
  ws.onclose = (event) => {
    ...
    console.log('[Alma Relay] WebSocket closed:', event.code, event.reason);
    connectionState = 'disconnected';
    ws = null;
  ...
  ws.onerror = (error) => {
    console.error('[Alma Relay] WebSocket error:', error);
  };
}
  ...
  pingTimer = setInterval(() => {
    if (ws && ws.readyState === WebSocket.OPEN) {
      ...
      if (Date.now() - lastPongTime > 60000) {
        console.warn('[Alma Relay] Pong timeout, reconnecting...');
        ws.close();
        return;
      }
      send({ type: 'ping' });
...
function setupDebuggerListeners() {
  ...
  chrome.debugger.onDetach.addListener((source, reason) => {
    if (source.tabId) {
      ...
      sendStatus();
      console.log(`[Alma Relay] Debugger detached from tab ${source.tabId}: ${reason}`);
    }
  });
}
...
async function handleMessage(raw) {
  ...
  try {
    ...
    console.error('[Alma Relay] Invalid JSON:', raw);
    return;
  }
